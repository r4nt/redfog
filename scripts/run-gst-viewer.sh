#!/usr/bin/env bash
# Builds (if needed) and runs the gst-wayland-display backend's standalone
# viewer/prover — the equivalent of kwin-viewer for the gst-backend crate.
# Idempotent: skips cloning/building steps that are already done.
#
# Usage:
...
echo "building gst-viewer..."
(cd "$REPO_DIR" && cargo build -p gst-backend --bin gst-viewer)

RUNTIME_DIR="${REDFOG_RUNTIME_DIR:-/tmp/gst-viewer-runtime}"
mkdir -p "$RUNTIME_DIR"

echo "starting gst-viewer ${WIDTH}x${HEIGHT}, nested: ${NESTED_COMMAND[*]}"
...
# against gst-viewer's own CWD, which otherwise depends on wherever the
# caller happened to invoke this script from (confirmed live: sway silently
# fails to find its config and falls back to its own default when run from
...
"$REPO_DIR/target/debug/gst-viewer" "$WIDTH" "$HEIGHT" "${NESTED_COMMAND[@]}"
