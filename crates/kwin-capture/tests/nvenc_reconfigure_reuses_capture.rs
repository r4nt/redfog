...
async fn reconfigure_keeps_video_flowing() {
    ...
    let session = CudaDirectEncoderSession::spawn(node_id, socket_path, 1280, 720, 60, 5_000, VideoCodec::H264, move |data, is_keyframe| {
...
async fn hevc_survives_many_frames_and_request_keyframe_produces_a_real_idr() {
    ...
    let (tx, rx) = std::sync::mpsc::channel::<(Vec<u8>, bool)>();
    let session = CudaDirectEncoderSession::spawn(node_id, socket_path, 1280, 720, 60, 5_000, VideoCodec::Hevc, move |data, is_keyframe| {
        let _ = tx.send((data, is_keyframe));
    });
